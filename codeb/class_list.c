...
class_node_t *addItem(class_node_t *list, char *className, char *methName, char *functionName, short kind)
{
    if (list == NULL)
    {
...
class_node_t *addLocalVar(class_node_t *list, char *className, char *functionName, char *localVarName)
{
    return addItem(list, className, localVarName, functionName, KIND_LOCAL_VAR);
}
...
int countLocalVarsForClassMethod(class_node_t *list, char *className, char *functionName)
{
    class_node_t *nextNode = list;
    int result = 0;
    while (nextNode != NULL)
    {
        if (strcmp(nextNode->className, className) == 0 &&
            strcmp(nextNode->functionName, functionName) == 0 &&
