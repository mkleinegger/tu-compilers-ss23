...
int getKindOfName(node_t *list, char *name, int lineNr)
{
    ...
    fprintf(stderr, "Identifier with name '%s' on line %d is has no valid kind\n",
            name, lineNr);
    exit(3);
}
...
int getOffsetForSelector(node_t *selectors, char *methName)
{
    ...
    node_t *nextNode = selectors;
    while (nextNode != NULL)
    {
        if (strcmp(nextNode->name, methName) == 0)
        {
            return result;
        }
        else if (nextNode->kind == KIND_SELECTOR)
        {
            result++;
        }
