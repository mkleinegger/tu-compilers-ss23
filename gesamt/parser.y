...
@attributes { struct node_t *ids; struct type_t type; tree_t *tree; class_node_t* classList; } Expr RepExpr Term OrTerm Negations PlusTerm MulTerm 
...
Stat            : Return
                ...
                @{
                    ...
                    @i @Stat.0.tree@ = createNode(OP_ASSIGN, createIdentifierLeaf(@ID.name@, getKindOfName(@Stat.0.out@, @ID.name@, @ID.lineNr@), getParameterIndex(@Stat.0.out@, @ID.name@), getLocalVarOffset(@Stat.0.out@, @ID.name@)), @Expr.0.tree@);
                    @reg {
                ...
                @{
                    ...
                    @i @Stat.0.tree@ = createNode(OP_ASSIGN, createIdentifierLeaf(@ID.name@, getKindOfName(@Stat.0.out@, @ID.name@, @ID.lineNr@), getParameterIndex(@Stat.0.out@, @ID.name@), getKindOfName(@Stat.0.out@, @ID.name@, @ID.lineNr@) == KIND_VARIABLE ? getLocalVarOffset(@Stat.0.out@, @ID.name@) : getClassVarOffset(@Stat.0.out@, @ID.name@)), @Expr.0.tree@);
                    @reg {
                        @Stat.0.tree@->regStor = getFirstRegister(); 
                        @Expr.0.tree@->regStor = @Stat.0.tree@->regStor;
                    }
...
Guarded         : RIGHTASSIGN Stats BREAK 
                ...
                @{
                    ...
                    @i @Guarded.0.condCounterOut@ = @Stats.0.condCounterOut@;
                    @i @Guarded.0.tree@ = createNode(OP_COND, createIfLabelLeaf(prepareLabelString("label", @Guarded.className@, @Guarded.functionName@, globalGuardCounter++)), @Expr.tree@);
                    @reg {
                        @Guarded.0.tree@->regStor = getFirstRegister(); 
                        @Expr.0.tree@->regStor = @Guarded.0.tree@->regStor;
                    }
                    ...
                    /* write jump-instruction to jump to the beginning of cond */
                    @burm {
                        writeJumpTo(prepareLabelString("cond", @Guarded.className@, @Guarded.functionName@, @Stats.0.condCounterIn@ ));
                        writeEndLabel(@Guarded.0.tree@->kids[0]->identifierName);
                    }
                @}
                ;
...
RepExpr         : Expr
                @{
                    ...
                    @i @RepExpr.0.tree@ =  createNode(OP_ONE_METHOD_EXPR, @Expr.0.tree@, NULL);
                ...
                @{
                    ...
                    @i @RepExpr.0.tree@ = createNode(OP_METHOD_EXPR, @Expr.0.tree@, @RepExpr.1.tree@);
                    @reg {
                        @Expr.0.tree@->regStor = getFirstRegister();
                        @RepExpr.1.tree@->regStor = getNextParameterRegister(@RepExpr.0.tree@->regStor);   
                    }                
                @}
                ;
...
Term            : '(' Expr ')'
                ...
                @{
                    ...
                    @i @Term.0.tree@ = createIdentifierLeaf(@ID.name@, getKindOfName(@Term.0.ids@, @ID.name@, @ID.lineNr@), getParameterIndex(@Term.0.ids@, @ID.name@), getKindOfName(@Term.ids@, @ID.name@, @ID.lineNr@) == KIND_VARIABLE ? getLocalVarOffset(@Term.ids@, @ID.name@) : getClassVarOffset(@Term.ids@, @ID.name@));
                @}
                | ID '(' RepExpr ')' /* Aufruf */
                @{                    
                    ...
                    @i @Term.0.tree@ = createNode(OP_METHOD, createMethodLeaf(@ID.name@, getMaxParameterIndex(@Term.0.ids@)), @RepExpr.0.tree@);
                    @reg @RepExpr.0.tree@->regStor = getFirstParameterRegister();
                @}
                ;
